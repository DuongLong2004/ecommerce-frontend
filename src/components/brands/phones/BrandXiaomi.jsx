...
const buildServerParams = (searchParams) => {
  ...
  const params = { category: "phone", brand: "Xiaomi", limit: 100 };
...
const BrandXiaomiPage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load Xiaomi:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .xiaomi-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .xiaomi-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
      <div className="xiaomi-grid">
        {products.map((item) => (
...
export default BrandXiaomiPage;