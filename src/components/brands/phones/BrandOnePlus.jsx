...
const buildServerParams = (searchParams) => {
  ...
  const params = { category: "phone", brand: "OnePlus", limit: 100 };
...
const BrandOnePlusPage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load OnePlus:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .oneplus-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .oneplus-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
      <div className="oneplus-grid">
        {products.map((item) => (
...
export default BrandOnePlusPage;