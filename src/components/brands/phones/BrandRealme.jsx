...
const buildServerParams = (searchParams) => {
  ...
  const params = { category: "phone", brand: "Realme", limit: 100 };
...
const BrandRealmePage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load Realme:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .realme-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .realme-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
      <div className="realme-grid">
        {products.map((item) => (
...
export default BrandRealmePage;