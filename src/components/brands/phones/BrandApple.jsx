...
const buildServerParams = (searchParams) => {
  ...
  const params = { category: "phone", brand: "Apple", limit: 100 };
...
const BrandApplePage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load Apple:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .apple-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .apple-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
