...
const buildServerParams = (searchParams) => {
  ...
  const params = {
    category: "phone",
    brand: "Samsung", // ✅ cố định brand Samsung
    limit: 100,
  };
...
const BrandSamsungPage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load Samsung phones:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .samsung-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .samsung-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
...
export default BrandSamsungPage;