...
const buildServerParams = (searchParams) => {
  ...
  const params = { category: "phone", brand: "Sony", limit: 100 };
...
const BrandSonyPage = () => {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("Lỗi load Sony:", err);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 1024px) {
          .sony-grid { grid-template-columns: repeat(auto-fill, minmax(160px, 1fr)); gap: 12px; }
        }
        @media (max-width: 640px) {
          .sony-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; padding: 12px; }
        }
      `}</style>
