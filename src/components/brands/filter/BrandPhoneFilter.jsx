...
/**
 * BrandPhoneFilter
 * Dùng cho trang thương hiệu cụ thể (Apple, Samsung, OPPO, ...)
 * Props:
 *   - brand: string — tên hãng hiển thị trong header (vd: "Apple", "Samsung")
 */
...
const filterOptions = {
  ...
  chip: [
    ...
    { label:"Apple A16",     value:"A16"      },
    { label:"Apple A15",     value:"A15"      },
  ],
  camera:  [{ label:"48 MP",value:"48MP"},{label:"50 MP",value:"50MP"},{label:"200 MP",value:"200MP"}],
  battery: [{ label:"3400 mAh",value:"3400"},{label:"4422 mAh",value:"4422"},{label:"4670 mAh",value:"4670"},{label:"5000 mAh",value:"5000"}],
};
...
/* ─── COMPONENT ─────────────────────────────────────────────────────── */
export default function BrandPhoneFilter({ brand = "iPhone" }) {
  ...
  return (
    <>
      <style>{`
        ...
        .bpf { font-family:'Nunito',sans-serif; width:100%; }
        .bpf * { box-sizing:border-box; font-family:inherit; }
        .bpf button { cursor:pointer; border:none; outline:none; }
        .bpf button:active { opacity:.85; }
        @keyframes bpf-drop { from{opacity:0;transform:translateY(-5px)} to{opacity:1;transform:translateY(0)} }
        ...
        /* header */
        .bpf-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:13px; }
        .bpf-hleft  { display:flex; align-items:center; gap:10px; }
        .bpf-hicon  {
        ...
        .bpf-htitle { font-size:13px; font-weight:800; color:#111; line-height:1.3; }
        .bpf-hsub   { font-size:10px; color:#bbb; margin-top:1px; }
        .bpf-badge  { font-size:10px; font-weight:800; padding:2px 9px; border-radius:20px; background:#f4f4f4; color:#bbb; transition:all .2s; white-space:nowrap; }
        .bpf-badge.on { background:#dc2626; color:#fff; }
        .bpf-div { height:1px; background:#f5f5f5; margin:0 0 14px; }
        ...
        .bpf-sbar { width:3px; height:11px; border-radius:2px; background:#dc2626; flex-shrink:0; }

        /* dropdowns */
        .bpf-drops { display:grid; grid-template-columns:1fr 1fr; gap:6px; margin-bottom:4px; }
        .bpf-dw    { position:relative; }
        .bpf-dw:last-child:nth-child(odd) { grid-column: span 2; }
        .bpf-dbtn {
        ...
        .bpf-dbtn:hover    { border-color:#fca5a5; background:#fff8f8; }
        .bpf-dbtn.open     { border-color:#dc2626; background:#fff5f5; border-radius:8px 8px 0 0; }
        .bpf-dbtn.act      { border-color:#fecaca; background:#fff8f8; }
        .bpf-dbtn.open.act { border-color:#dc2626; }
        .bpf-dlbl { flex:1; text-align:left; font-size:11.5px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
        .bpf-dcnt {
        ...
        .bpf-panel {
          ...
          padding:4px 4px 6px; animation:bpf-drop .15s ease;
        }
        .bpf-opt {
        ...
        .bpf-opt:hover { background:#fff0f0; }
        .bpf-opt.on    { background:#fff0f0; color:#dc2626; font-weight:700; }
        .bpf-chk {
          width:14px; height:14px; border-radius:3.5px; flex-shrink:0;
          border:1.5px solid #ddd; background:#fff;
          display:flex; align-items:center; justify-content:center; transition:all .14s;
        }
        .bpf-chk.on { background:#dc2626; border-color:#dc2626; color:#fff; }

        /* tags */
        .bpf-tags      { margin-top:14px; padding:10px 9px; background:#fafafa; border-radius:8px; border:1px dashed #f5c6c6; }
        .bpf-tags-ttl  { font-size:9.5px; font-weight:800; color:#ccc; text-transform:uppercase; letter-spacing:.06em; margin-bottom:7px; display:flex; align-items:center; gap:5px; }
        .bpf-tags-list { display:flex; flex-wrap:wrap; gap:5px; }
        .bpf-tag       { display:inline-flex; align-items:center; gap:3px; padding:3px 7px 3px 9px; background:#fff; border:1px solid #fecaca; border-radius:20px; font-size:11px; }
        .bpf-tag-k     { color:#bbb; font-size:10px; }
        .bpf-tag-v     { font-weight:700; color:#111; margin-left:1px; }
        .bpf-tag-del   { width:15px; height:15px; border-radius:50%; background:#fee2e2; color:#dc2626; display:flex; align-items:center; justify-content:center; margin-left:3px; flex-shrink:0; padding:0; transition:background .15s; }
        .bpf-tag-del:hover { background:#fca5a5; }
        .bpf-clear     { display:inline-flex; align-items:center; gap:4px; padding:3px 10px; background:transparent; border:1px solid #fca5a5 !important; border-radius:20px; color:#dc2626; font-size:11px; font-weight:700; transition:background .15s; }
        .bpf-clear:hover { background:#fff0f0; }

        @media (max-width:900px) {
          .bpf-drops { grid-template-columns:repeat(3,1fr); }
        }
        @media (max-width:600px) {
          .bpf-drops { grid-template-columns:repeat(2,1fr); }
        }
      `}</style>

      <div className="bpf" ref={ref}>
        <div className="bpf-wrap">

          {/* HEADER */}
          <div className="bpf-header">
            <div className="bpf-hleft">
              <div className="bpf-hicon"><SvgFilter /></div>
              <div>
                <div className="bpf-htitle">Lọc {brand}</div>
                <div className="bpf-hsub">Tìm đúng máy bạn cần</div>
              </div>
            </div>
            <div className={`bpf-badge ${total > 0 ? "on" : ""}`}>
              {total > 0 ? `${total} bộ lọc` : "0 bộ lọc"}
            </div>
          </div>
          <div className="bpf-div" />

          {/* BRAND BADGE CỐ ĐỊNH */}
          <div className="bpf-brand-badge">
            {brand === "iPhone" || brand === "Apple" ? <AppleLogo /> : null}
            {brand}
          </div>

          {/* DROPDOWNS */}
          <div className="bpf-slabel"><span className="bpf-sbar"/><span>Bộ lọc chi tiết</span></div>
          <div className="bpf-drops">
            {KEYS.map(k => {
              const { label, icon } = filterMeta[k];
              const open   = openKey === k;
              const active = hasVal(k);
              return (
                <div key={k} className="bpf-dw">
                  <button className={`bpf-dbtn ${open?"open":""} ${active?"act":""}`}
                    onClick={() => setKey(p => p===k ? null : k)}>
                    <span style={{ color:active?"#dc2626":"#ccc", display:"flex", flexShrink:0 }}>
                      {iconMap[icon]}
                    </span>
                    <span className="bpf-dlbl" style={{ color:active?"#dc2626":"#444" }}>{label}</span>
                    {cnt(k) > 0 && <span className="bpf-dcnt">{cnt(k)}</span>}
                    <span style={{ display:"flex", flexShrink:0, color:active?"#dc2626":"#ccc" }}>
                      <SvgChevron open={open}/>
                    </span>
                  </button>
                  {open && (
                    <div className="bpf-panel">
                      {filterOptions[k].map(opt => {
                        const on = isOn(k, opt.value);
                        return (
                          <button key={opt.label+opt.value}
                            className={`bpf-opt ${on?"on":""}`}
                            onClick={() => toggle(k, opt.value)}>
                            <span className={`bpf-chk ${on?"on":""}`}>
                              {on && <SvgCheck size={9}/>}
                            </span>
                            {opt.label}
                          </button>
          ...
          {total > 0 && (
            <div className="bpf-tags">
              ...
              <div className="bpf-tags-list">
                {tags.map((t,i) => (
                  <span key={i} className="bpf-tag">
                    <span className="bpf-tag-k">{filterMeta[t.k]?.label}:</span>
                    <span className="bpf-tag-v">{fmtTag(t.k, t.v)}</span>
                    <button className="bpf-tag-del" onClick={() => rmTag(t.k, t.v)}><SvgX size={8}/></button>
                  </span>
                ))}
                <button className="bpf-clear" onClick={clearAll}>
